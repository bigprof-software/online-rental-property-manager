<?php
	...
	if($thisMD5==$prevMD5){
		...
		setupTable('property_photos', "create table if not exists `property_photos` (   `id` INT unsigned not null auto_increment , primary key (`id`), `property` INT unsigned , `photo` VARCHAR(40) , `description` TEXT ) CHARSET utf8", $silent);
		setupIndexes('property_photos', array('property'));
		...
		setupTable('unit_photos', "create table if not exists `unit_photos` (   `id` INT unsigned not null auto_increment , primary key (`id`), `unit` INT unsigned , `photo` VARCHAR(40) , `description` TEXT ) CHARSET utf8", $silent);
		setupIndexes('unit_photos', array('unit'));
