...
function property_photos_insert(){
	global $Translation;
	...
	if($data['photo']) createThumbnail($data['photo'], getThumbnailSpecs('property_photos', 'photo', 'tv'));
	if($data['photo']) createThumbnail($data['photo'], getThumbnailSpecs('property_photos', 'photo', 'dv'));

	/* for empty upload fields, when saving a copy of an existing record, copy the original upload field */
	if($_REQUEST['SelectedID']){
		$res = sql("select * from property_photos where id='" . makeSafe($_REQUEST['SelectedID']) . "'", $eo);
	...
	if(function_exists('property_photos_before_insert')){
		$args=array();
		if(!property_photos_before_insert($data, getMemberInfo(), $args)){ return false; }
	}

	$o = array('silentErrors' => true);
	sql('insert into `property_photos` set       `property`=' . (($data['property'] !== '' && $data['property'] !== NULL) ? "'{$data['property']}'" : 'NULL') . ', ' . ($data['photo'] != '' ? "`photo`='{$data['photo']}'" : '`photo`=NULL') . ', `description`=' . (($data['description'] !== '' && $data['description'] !== NULL) ? "'{$data['description']}'" : 'NULL'), $o);
	if($o['error']!=''){
		echo $o['error'];
		echo "<a href=\"property_photos_view.php?addNew_x=1\">{$Translation['< back']}</a>";
		exit;
	}
	...
	set_record_owner('property_photos', $recID, getLoggedMemberID());
...
function property_photos_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	...
	$arrPerm=getTablePermissions('property_photos');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='property_photos' and pkValue='$selected_id'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='property_photos' and pkValue='$selected_id'");
...
function property_photos_update($selected_id){
	global $Translation;

	// mm: can member edit record?
	$arrPerm=getTablePermissions('property_photos');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='property_photos' and pkValue='".makeSafe($selected_id)."'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='property_photos' and pkValue='".makeSafe($selected_id)."'");
	...
	if(function_exists('property_photos_before_update')){
		$args=array();
		if(!property_photos_before_update($data, getMemberInfo(), $args)){ return false; }
	}

	$o=array('silentErrors' => true);
	sql('update `property_photos` set       `property`=' . (($data['property'] !== '' && $data['property'] !== NULL) ? "'{$data['property']}'" : 'NULL') . ', ' . ($data['photo']!='' ? "`photo`='{$data['photo']}'" : ($_REQUEST['photo_remove'] != 1 ? '`photo`=`photo`' : '`photo`=NULL')) . ', `description`=' . (($data['description'] !== '' && $data['description'] !== NULL) ? "'{$data['description']}'" : 'NULL') . " where `id`='".makeSafe($selected_id)."'", $o);
	if($o['error']!=''){
		echo $o['error'];
	...
	sql("update membership_userrecords set dateUpdated='".time()."' where tableName='property_photos' and pkValue='".makeSafe($selected_id)."'", $eo);

}

function property_photos_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	if($selected_id){
		...
		$res = sql("select * from `property_photos` where `id`='".makeSafe($selected_id)."'", $eo);
	...
	// process buttons
	if($AllowInsert){
		if(!$selected_id) $templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return property_photos_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
		$templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-default" id="insert" name="insert_x" value="1" onclick="return property_photos_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save As Copy'] . '</button>', $templateCode);
	}else{
	...
	if($selected_id){
		...
		if($AllowUpdate){
			$templateCode = str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return property_photos_validateData();" title="' . html_attr($Translation['Save Changes']) . '"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		}else{
	...
	if($selected_id){
		$jdata = get_joined_record('property_photos', $selected_id);
	...
	$templateCode .= loadView('property_photos-ajax-cache', $cache_data);

	// hook: property_photos_dv
	if(function_exists('property_photos_dv')){
		$args=array();
		property_photos_dv(($selected_id ? $selected_id : FALSE), getMemberInfo(), $templateCode, $args);
	}
