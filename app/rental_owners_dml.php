...
function rental_owners_insert(&$error_message = '') {
	...
	if(!empty($_REQUEST['SelectedID'])) rental_owners_copy_children($recID, $_REQUEST['SelectedID']);
...
function rental_owners_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	if($selected_id) {
		...
		$combo_state->SelectedText = ( $_REQUEST['FilterField'][1] == '12' && $_REQUEST['FilterOperator'][1] == '<=>' ? $_REQUEST['FilterValue'][1] : '');
	}
	...
	$templateCode .= loadView('rental_owners-ajax-cache', array('rdata' => $rdata, 'jdata' => $jdata));
