...
<script>
	$j(function() {
		...
		var data = {
			tenants: <?php echo json_encode(['id' => $rdata['tenants'], 'value' => $rdata['tenants'], 'text' => $jdata['tenants']], JSON_INVALID_UTF8_SUBSTITUTE); ?>,
			property: <?php echo json_encode(['id' => $rdata['property'], 'value' => $rdata['property'], 'text' => $jdata['property']], JSON_INVALID_UTF8_SUBSTITUTE); ?>,
			unit: <?php echo json_encode(['id' => $rdata['unit'], 'value' => $rdata['unit'], 'text' => $jdata['unit']], JSON_INVALID_UTF8_SUBSTITUTE); ?>
		};
