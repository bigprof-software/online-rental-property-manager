...
function applications_leases_insert(&$error_message = '') {
	...
	if(!empty($_REQUEST['SelectedID'])) applications_leases_copy_children($recID, $_REQUEST['SelectedID']);
...
function applications_leases_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	$filterer_tenants = thisOr($_REQUEST['filterer_tenants'], '');
	...
	if($selected_id) {
		...
		$combo_type->SelectedText = ( $_REQUEST['FilterField'][1] == '6' && $_REQUEST['FilterOperator'][1] == '<=>' ? $_REQUEST['FilterValue'][1] : 'Fixed');
		$combo_recurring_charges_frequency->SelectedText = ( $_REQUEST['FilterField'][1] == '10' && $_REQUEST['FilterOperator'][1] == '<=>' ? $_REQUEST['FilterValue'][1] : 'Monthly');
	}
	...
	$templateCode .= loadView('applications_leases-ajax-cache', array('rdata' => $rdata, 'jdata' => $jdata));
