...
function properties_insert(&$error_message = '') {
	...
	if(!empty($_REQUEST['SelectedID'])) properties_copy_children($recID, $_REQUEST['SelectedID']);
...
function properties_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	$filterer_owner = thisOr($_REQUEST['filterer_owner'], '');
	...
	if($selected_id) {
		...
		$combo_State->SelectedText = ( $_REQUEST['FilterField'][1] == '13' && $_REQUEST['FilterOperator'][1] == '<=>' ? $_REQUEST['FilterValue'][1] : '');
	}
	$combo_type->Render();
	...
	$templateCode .= loadView('properties-ajax-cache', array('rdata' => $rdata, 'jdata' => $jdata));
