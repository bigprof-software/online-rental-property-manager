...
function properties_insert(&$error_message = '') {
	...
	$recID = tableInsert('properties', $data, $recordOwner, $error_message);

	// if this record is a copy of another record, copy children if applicable
	if(strlen(Request::val('SelectedID')) && $recID !== false)
		properties_copy_children($recID, Request::val('SelectedID'));
...
function properties_update(&$selected_id, &$error_message = '') {
	...
	if(function_exists('properties_after_update')) {
		if($row = getRecord('properties', $data['selectedID'])) $data = array_map('makeSafe', $row);
...
function properties_form($selectedId = '', $allowUpdate = true, $allowInsert = true, $allowDelete = true, $separateDV = true, $templateDV = '', $templateDVP = '') {
	...
	if($hasSelectedId) {
		if(!($row = getRecord('properties', $selectedId))) {
		...
		<?php if($fieldsAreEditable) { ?>
			...
			$j("#owner-container__RAND__").select2({
				...
				$j(this).parents('.form-group')
					.find('.btn[id=rental_owners_view_parent]')
	...
	$lookupFields = getLookupFields()['properties'] ?? [];
